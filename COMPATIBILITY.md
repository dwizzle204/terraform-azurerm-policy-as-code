...
|-----------|---------|--------|-------|
| Terraform | >= 1.8 | 1.15.9 | Floor exists for the offline `terraform test` / `mock_provider` suites run in CI |
...
CI runs the offline module suite against two Terraform lines (`~1.10`, `~1.15`)
...
- `>= 1.8`: oldest maintained Terraform line the offline test suite is
  validated against (`mock_provider` requires >= 1.7).
